...
from models import Split_Autoencoder
...
def main():
    ...
    parser.add_argument('--alpha', type=float, default=0.5, metavar='a',
                        help='propotion of eucledian space loss (Default=0.5)')
    parser.add_argument('--gamma', type=float, default=0.0, metavar='g',
                        help='propotion of identity loss (Default=0.0)')
    ...
    writer = SummaryWriter(logging_dir, comment='Split Autoencoder for ants')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (data,rotations) in enumerate(train_loader):
            ...
            #Synthetically rotate input and get target and relative rotation in degrees

            targets,relative_rotations=rotate_tensor(args,data) 

            relative_rotations=relative_rotations.view(-1,1)

            # Forward pass (output, tuple of identity vectors, tuple rotation loss)
            output, identity_vectors, eucleidian_vectors= model(data, targets,relative_rotations*np.pi/180) 
            optimizer.zero_grad()
